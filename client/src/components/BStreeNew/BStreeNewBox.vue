<template>
  <div class="BStreeNewBox">
    <el-buttongroup class="btnGroup" v-if="btnGroup">
      <Button class="btnStyle" @click="throwData(4)" title="添加"><i class="iconfont icon-add"></i></Button>
      <Button class="btnStyle" @click="throwData(2)" :disabled="moveBtu('eid')" title="编辑"><i class="iconfont icon-edit1"></i></Button>
      <Button class="btnStyle" @click="throwData(3)" :disabled="moveBtu('del')" title="删除"><i class="iconfont icon-delete"></i></Button>
      <Button class="btnStyle" @click="nodeMove('up')" :disabled="moveBtu('up')" title="上移"><i class="iconfont icon-move-up"></i></Button>
      <Button class="btnStyle" @click="nodeMove('down')" :disabled="moveBtu('down')" title="下移"><i class="iconfont icon-move-down"></i></Button>
      <Button class="btnStyle" @click="refresh" title="刷新"><i class="iconfont icon-shuaxin"></i></Button>
    </el-buttongroup>
    ...
    <slot name='dialog' :orgName="orgName(isActive)" :visible="addVisible" :title="isNew ? '机构添加' : '机构修改'">
      <Dialog class="dialog" :visible="addVisible" :title="isNew ? '机构添加' : '机构修改'" width="450px" :close-on-click-modal="true" :modal-append-to-body="true" :append-to-body="false" @close="orgCancel">
        <div class="org-modal-form">
          <Form :model="formData" label-position="left" label-width="100px" ref="orgFormData" :rules="orgFormRole">
            <FormItem label="上级机构">
              <div class="ivu-form-item-content-span">{{orgName(isActive)}}</div>
            </FormItem>
            <FormItem label="机构编号" v-if="isNew" prop="code">
        ...
        <div slot="footer">
          ...
          <Button type="primary" @click="orgSave(formData)" :loading="modalloading" size="small">确认</Button>
        </div>
      </Dialog>
    </slot>
  </div>
      ...
      if (this.isActive.data !== undefined && this.isActive.data.isroot !== undefined) {
        if (data === 'up') {
          if (this.isActive.previousSibling !== null && this.isActive.previousSibling !== undefined) {
            if (this.isActive.previousSibling.data.isroot !== undefined) {
              // 是机构
              return false
            }
          }
          ...
          if (this.isActive.nextSibling !== null && this.isActive.nextSibling !== undefined) {
            if (this.isActive.nextSibling.data.isroot !== undefined) {
              // 是机构
              return false
            }
          }
              ...
              onOk: () => {
                ...
                this.$http
                  ...
                  .then(() => {
                    ...
                    this.$emit('delData', this.initdata._id)
