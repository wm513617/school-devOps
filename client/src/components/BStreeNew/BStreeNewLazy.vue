<template>
  <div class="BStree">
    <!-- 懒加载树 -->
    <Tree style="height:100%;" :highlight-current="true" :current-node-key="currentNodeKey" ref="treeNewLazy" :props="defaultProps" :load="loadNode" lazy :default-expanded-keys="nodeKey" @node-click="nodeClick" :show-checkbox="checkBox" :node-key="id" draggable :allow-drag="()=>{return true}" :allow-drop="()=>{return false}" @check="handleCheckChange" @node-drag-start="handleDragStart" :filter-node-method="filterNode" :indent="10" @node-expand="expand" @node-collapse="expand">
