<template>
  <div>
    <bsr-tree :treeData="treeData" :showCheckbox="showCheckbox" :selectNode="selectNode" v-model="value" :isSearch="isSearch" :searchedIds="searchedIds" :searchVal="searchVal" ref="bsrtree" @node-click="item => $emit('node-click', item)" @node-dblclick="item => $emit('node-dblclick', item)" @on-expand="$emit('on-expand')" @handlechecked="item => $emit('handlechecked', item)" v-show="searching||!(searchVal&&searchedIds.length===0)">
