<template>
  <div class="treeNode" :class="[]" :draggable="true" @dragstart.stop="handleDragStart(treeData,$event)" @dblclick="dblclickNode(treeData)" v-if="showNode(treeData)&&treeData[label]&&visible">
    <span class="treeliBox" :class="[treeData._id === activeNode._id ? 'active': '',(searched)?'searched': '']" @click="handleNode(treeData)">

      <span :style="{'width':left + 'px','height':'100%'}" class="suojin"></span>

      <i @click.stop='handleNodeExpand(treeData)' class="openflag" :class="[open? 'tree-open':'tree-close','icon']" v-if="treeData.children && treeData.children.length > 0">
        <!-- <Icon type="arrow-down-b" v-if='open'></Icon> -->
        <i class="el-icon-caret-bottom" v-if='open'></i>
        <!-- <Icon type="arrow-right-b" v-if='!open'></Icon> -->
        <i class="el-icon-caret-right" v-if='!open'></i>
      </i>
      <!--无子节点缩进对齐-->
      ...
      <span class="checkbox" v-if="showCheckbox" >
        <span class="el-checkbox__input" :class="{'is-checked':checked,'is-indeterminate':halfChecked, 'is-disabled':!selective(treeData)}" @click.stop="handlecheckedChange(treeData)">
...
.treeNode .treeliBox {
  ...
  position: relative;
  line-height: 34px;
  background: @operation_background_tree
}
...
.treeNode .treeliBox:hover {
  background: @tree_background_hover;
}
...
.treeNode .treeliBox .item.offline {
  color: @btn-disable-bg-color;
}

.treeNode .treeliBox .item.playing {
  color: @app-container-bg-color;
}
...
.treeliBox.searched,.treeliBox.searched .item {
  /* background: #449af8; */
  color: #449af8;
}
