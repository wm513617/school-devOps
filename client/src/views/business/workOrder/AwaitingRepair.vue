<template>
  <div class="awaiting-repair" v-resize="resize" ref='bsMain'>
    <div class="ar-button">
      ...
      <DatePicker v-model="DatePickerValue" type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" size="small" :clearable="false" class="div-datetime"></DatePicker>
      <Button type="primary" size="small" icon="el-icon-search" @click="search">搜索</Button>
    </div>
    <div class="ar-table">
      <Table :data="workOrderData" style="width: 100%" :height="tableHeight" @selection-change="handleSelectionChange" highlight-current-row>
...
import { getWorkOrderListApi, removeEditWorkOrderApi } from '../../../http/workOrder.api'
export default {
  components: {
    // Button,
    // Input,
  ...
  data() {
    return {
      ...
      tableHeight: 420,
      loading: false,
      DatePickerValue: [this.$moment().subtract(7, 'days'), this.$moment()],
      limit: 25,
      page: 1,
      bscCount: 0,
  ...
  methods: {
    ...
    editOpenModal() { // 修改工单
      if (this.ids.length === 1) {
        ...
        this.$notify.warning({ message: '请选择一个工单' })
      }
    },
    ...
    cancel(val) { // 关闭工单弹窗
      this.DatePickerValue = [this.$moment().subtract(7, 'days'), this.$moment()]
    ...
    removeOrder() {
      if (this.ids.length < 1) {
        this.$notify.warning({ message: '请选择工单' })
        return
      }
    ...
    getData(val) {
      if (!this.DatePickerValue) {
        this.$notify.warning({ message: '请选择时间！' })
        return
      }
      this.loading = true
      const param = {
      ...
      getWorkOrderListApi(param).then(res => {
        ...
        this.loading = false
        if (val) {
          this.$notify.success({ message: val + '成功' })
        }
      }).catch(err => {
        console.log(err, 'err')
        this.loading = false
        if (val) {
          this.$notify.error({ message: val + '失败' })
        }
      })
    }
  },
